...
mlflow_exp_name = mlflow_params['exp_name']
...
def run(model_idx):
    ...
    mlflow_runs = mlflow.search_runs(filter_string=f'run_name = "exp_{args.experiment}_run_{model_idx}"')
    run_id = mlflow_runs['run_id'][0]
    with mlflow.start_run(run_id) as mlflow_current_run:
        model_path = mlflow_current_run.data.params['run/model_path']
        ...
        mlflow.log_param('prediction/n_images', n_processed_images)
        mlflow.log_param('prediction/mean_time', m_time)
