...
if __name__ == '__main__':
    with open(outfile, 'w') as sys.stdout:
        ...
        models_results = results_df.groupby(['model_idx'])[[
            'no_cloud_tns',
            'no_cloud_tps',
            'no_cloud_fns',
            'no_cloud_fps',
        ...
        for subset in ['no_cloud', 'cloud', 'global']:
            models_results[f'{subset}_precision'] = models_results[f'{subset}_tps'] / (models_results[f'{subset}_tps'] + models_results[f'{subset}_fps'])
            models_results[f'{subset}_recall'] = models_results[f'{subset}_tps'] / (models_results[f'{subset}_tps'] + models_results[f'{subset}_fns'])
            models_results[f'{subset}_f1'] = 2 * models_results[f'{subset}_precision'] * models_results[f'{subset}_recall'] / (models_results[f'{subset}_precision'] + models_results[f'{subset}_recall'])

        
        results_file = path_results / f'results_{args.experiment}.xlsx'
        with pd.ExcelWriter(results_file) as writer:     
            results_df.to_excel(writer, sheet_name='general results')
            models_results.to_excel(writer, sheet_name='models results')
            mean_results_df.to_excel(writer, sheet_name='mean results')
