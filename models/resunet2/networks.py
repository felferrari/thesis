...
class ResUnetEF(GenericResunet):    
    def __init__(self, *args, **kargs):
        super().__init__(*args, **kargs)
        self.prepare_model(self.opt_input + self.sar_input)

    def prepare_input(self, x):
        x_img_0 = self.get_opt(x)
        x_img_1 = self.get_sar(x)
        x = torch.cat((x_img_0, x_img_1), dim=1)
        return x
...
class ResUnetJF(GenericModel):
    ...
    def forward(self, x):
        ...
        x_opt = self.encoder_opt(x_img_opt)
        x_sar = self.encoder_sar(x_img_sar)
        x = []
