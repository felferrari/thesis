...
class ResUnetDecoder(nn.Module):
    def __init__(self, depths):
        ...
        self.dec_blocks = nn.ModuleList(
            [nn.Conv2d(depths[i-1] + depths[i], depths[i-1], kernel_size=1) for i in range(1, len(depths))]
        )
...
class ResUnetDecoderJF(nn.Module):
    def __init__(self, depths):
        ...
        self.dec_blocks = nn.ModuleList(
            [nn.Conv2d(2*depths[i-1] + depths[i], depths[i-1], kernel_size=1) for i in range(1, len(depths)-1)]
        )
        
        self.dec_blocks.append(
            nn.Conv2d(2*depths[-2] + 2*depths[-1], depths[-2], kernel_size=1)
        )
